...
Quick Setup
Docker + ROS
# Pull ARM64-native ROS 1 Noetic base image
...
rosrun phase_detection train_phases.py


Test the model

rosrun phase_detection test_phases.py
...
Why U-Net? Efficient with small datasets, captures local & global features via skip connections. Standard for microscopy & materials science.

Notes

Only 4 labeled images: model demonstrates correct pipeline but overfits.

torch.no_grad() ensures test inference does not update weights.

Rare phases (e.g., Phase B) are upweighted to handle class imbalance.
